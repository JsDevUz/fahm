...
import {
  ArrowLeft, RotateCcw, ChevronRight, Check, X,
  Layers, ListChecks, Type, AlignLeft, ArrowLeftRight, Play, BookOpen, Keyboard
} from 'lucide-react';
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px' }}>
      <div style={{
        ...
        <div style={{
          ...
          width: `${(current / total) * 100}%`, transition: 'width 0.35s ease',
        }} />
      </div>
      <span style={{ fontSize: '12px', color: 'var(--text-muted)', minWidth: '48px', textAlign: 'right' }}>
        {current + 1} / {total}
      </span>
    </div>
  );
...
function SettingsScreen({ onStart, onBack }) {
  ...
  const OptionCard = ({ active, onClick, icon: Icon, label, sub }) => (
    ...
    >
      ...
      {sub && (
        <span style={{ fontSize: '11px', color: 'var(--text-muted)', lineHeight: 1.2 }}>{sub}</span>
      )}
    </button>
  );
  ...
  return (
    <div className="animate-slide-in" style={{ maxWidth: '460px', margin: '0 auto' }}>
      <button className="btn btn-ghost" onClick={onBack} style={{ marginBottom: '24px' }}>
        <ArrowLeft size={15} /> Orqaga
      </button>

      <div className="glass-card" style={{ padding: '28px' }}>
        <h2 style={{ fontSize: '17px', fontWeight: 700, marginBottom: '4px' }}>Mashiq sozlamalari</h2>
...
function ResultScreen({ known, total, answers, onFinish }) {
  ...
  return (
    <div className="animate-slide-in" style={{ maxWidth: '400px', margin: '0 auto' }}>
      <div className="glass-card" style={{ padding: '32px', textAlign: 'center' }}>
        ...
        <h2 style={{ fontSize: '18px', fontWeight: 700, marginBottom: '20px' }}>Natija</h2>
        ...
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', marginBottom: '20px' }}>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '28px', fontWeight: 800, color: '#16a34a' }}>{known}</div>
            <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>to'g'ri</div>
          </div>
          <div style={{ width: '1px', background: 'var(--border-color)' }} />
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '28px', fontWeight: 800, color: 'var(--text-muted)' }}>{total - known}</div>
            <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>xato</div>
          </div>
          <div style={{ width: '1px', background: 'var(--border-color)' }} />
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '28px', fontWeight: 800, color: 'var(--accent-primary)' }}>{pct}%</div>
            <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>ball</div>
          </div>
        </div>

        <div style={{ display: 'flex', gap: '10px' }}>
          <button className="btn btn-ghost" onClick={onFinish} style={{ flex: 1, padding: '11px' }}>
            <RotateCcw size={14} /> Qayta
          </button>
          <button className="btn btn-primary" onClick={onFinish} style={{ flex: 1, padding: '11px' }}>
            Tugatish
          </button>
        </div>
      </div>
...
function FlashcardMode({ cards, direction, onFinish }) {
  ...
  if (done) {
    return <ResultScreen known={known} total={cards.length} answers={null} onFinish={onFinish} />;
  }

  return (
    <div className="animate-slide-in" style={{ maxWidth: '500px', margin: '0 auto' }}>
      ...
      <div
        ...
        style={{
          minHeight: '200px', borderRadius: '14px', cursor: 'pointer',
          border: '1.5px solid ' + (flipped ? 'var(--accent-primary)' : 'var(--border-color)'),
          background: flipped ? '#1e3a5f' : 'var(--bg-secondary)',
          boxShadow: 'var(--shadow-sm)',
          display: 'flex', flexDirection: 'column',
          alignItems: 'center', justifyContent: 'center',
          padding: '28px', transition: 'background 0.25s, border 0.25s',
          userSelect: 'none',
        }}
      >
        {!flipped ? (
          <>
            <LangLabel isArabic={isFrontArabic} style={{ marginBottom: '14px' }} />
            <div style={{
          ...
          <>
            <LangLabel isArabic={isBackArabic} style={{ marginBottom: '14px', color: 'rgba(255,255,255,0.45)' }} />
            <div style={{
            ...
            {card.hint && (
              <div style={{
                ...
                marginTop: '12px', textAlign: 'center', maxWidth: '320px', lineHeight: 1.6,
              }}>
                {card.hint}
              </div>
            )}
      ...
      {/* Action buttons */}
      {flipped ? (
        <div className="animate-fade-in" style={{ display: 'flex', gap: '10px', marginTop: '14px' }}>
          <button
            onClick={() => next('unknown')}
            style={{
              flex: 1, padding: '13px', borderRadius: '10px', cursor: 'pointer',
              background: 'var(--bg-secondary)', color: '#dc2626',
          ...
          <button
            ...
            style={{
              flex: 1, padding: '13px', borderRadius: '10px', cursor: 'pointer',
              background: 'var(--bg-secondary)', color: '#16a34a',
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '14px', gap: '10px' }}>
          <button className="btn btn-ghost" onClick={onFinish} style={{ padding: '9px 14px' }}>
...
function TestMode({ cards, direction, onFinish }) {
  ...
  if (done) {
    return <ResultScreen known={score} total={cards.length} answers={answers} onFinish={onFinish} />;
  }
    ...
    <div className="animate-slide-in" style={{ maxWidth: '500px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: '2px', flex: 1 }}>
          {cards.map((_, i) => (
            <div key={i} style={{
              ...
              background: i < idx
                ? (answers[i]?.correct ? '#16a34a' : '#e5e7eb')
                : i === idx ? 'var(--accent-primary)' : 'var(--bg-tertiary)',
              transition: 'background 0.3s',
            }} />
          ))}
        </div>
        <span style={{ fontSize: '12px', color: 'var(--text-muted)', minWidth: '44px', textAlign: 'right' }}>
          {idx + 1}/{cards.length}
        </span>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
        {options.map((opt, oi) => {
          ...
          const isCorrect  = opt.uz === card.uz && opt.ar === card.ar;
          const isSelected = selected && selected.uz === opt.uz && selected.ar === opt.ar;
          const isHighlighted = highlighted === oi;

          let border = isHighlighted && !selected ? 'var(--accent-primary)' : 'var(--border-color)';
          let color  = 'var(--text-primary)';
          let bg     = isHighlighted && !selected ? 'var(--accent-light)' : 'var(--bg-secondary)';
          if (selected) {
            if (isCorrect)       { border = '#86efac'; color = '#15803d'; bg = '#f0fdf4'; }
            else if (isSelected) { border = '#fca5a5'; color = '#b91c1c'; bg = '#fef2f2'; }
            else                 { border = 'var(--border-color)'; bg = 'var(--bg-secondary)'; color = 'var(--text-muted)'; }
          }
      ...
      {/* Skip after answer */}
      {selected && (
        <div className="animate-fade-in" style={{ marginTop: '10px', textAlign: 'right' }}>
          <button className="btn btn-ghost" onClick={skipToNext} style={{ padding: '8px 14px', fontSize: '13px' }}>
    ...
    <div style={{ maxWidth: '760px', margin: '0 auto', paddingBottom: '40px' }}>
      ...
      {phase === 'playing' && config && (
        config.mode === 'flashcard'
          ? <FlashcardMode cards={cards} direction={config.direction} onFinish={handleFinish} />
          : <TestMode      cards={cards} direction={config.direction} onFinish={handleFinish} />
      )}
    </div>
  );
}
